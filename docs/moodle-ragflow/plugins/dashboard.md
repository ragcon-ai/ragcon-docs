...
??? example "Enterprise — cost control &amp; chargeback"
    ...
    to set a budget, spot a runaway course early, and attribute cost to the department that owns it.
...
??? example "Universities — adoption evidence &amp; oversight"
    ...
    the success rate over time — the evidence a rollout committee needs to justify and steer it.
...
??? example "Public sector — accountability without storing content"
    ...
    CSV, XML or PDF for the record.

    The Status tab tells *misconfigured* apart from *RAGflow unreachable*, so
